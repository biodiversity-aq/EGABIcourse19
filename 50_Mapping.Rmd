...
```{r somap_pkgs, echo = FALSE, message = FALSE, warning = FALSE}
...
```{r somap_raster1, message = FALSE, warning = FALSE, cache = TRUE}
library(raster)
...
```{r soauto3, cache = TRUE}
SOmap_auto(xr)
```

But we can add the raster if we wish:
```{r soauto4, cache = TRUE}
SOmap_auto(xr)
...
```{r somap6, message = FALSE, warning = FALSE, cache = TRUE}
library(ggplot2)
...
## change the arguments being passed to the bathy plotting function
temp$bathy[[1]]$plotargs <- list(data = ice_raster_as_df, mapping = aes_string(fill = "ice"))
...
```{r somap5f, include = FALSE, eval = FALSE}
library(ggnewscale)
