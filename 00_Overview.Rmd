...
```{r pkgs2}
library(remotes)
pkgs <- c("SCAR/antanym", "AustralianAntarcticDivision/blueant",
...
library(SOmap)
SOauto_map(x$decimalLongitude, x$decimalLatitude, input_lines = FALSE, contours = FALSE, 
           pcol = "black", pcex = 0.2)
...
basemap <- SOmap(Trim = ceiling(max(x$decimalLatitude))+1, Bathleg = FALSE)
plot(basemap)
...
Reorganise data into presence/absence by sampling site:
...
```{r mod1, cache = TRUE, message = FALSE}
library(mgcv)
...
```{r mod2, cache = TRUE, message = FALSE}
...
my_cmap <- if (getRversion() >= "3.6") hcl.colors(21, "Geyser") else topo.colors(21)
```

Plot it:

```{r predmap2}
p <- SOauto_map(x = pr, contours = FALSE, bathy = pr)
